...
export async function POST(req: Request) {
  ...
  const tables: [string, string][] = [
    ...
    ["TABLE events", `CREATE TABLE IF NOT EXISTS events (id UUID PRIMARY KEY DEFAULT gen_random_uuid(), user_id UUID NOT NULL REFERENCES users(id) ON DELETE CASCADE, title TEXT NOT NULL, subtitle TEXT, description TEXT, thumbnail TEXT, stream_type stream_type_key NOT NULL, stream_key TEXT, rtmp_url TEXT, hls_url TEXT, youtube_url TEXT, youtube_channel_name TEXT, youtube_broadcast_id TEXT, youtube_stream_id TEXT, embed_code TEXT, status event_status NOT NULL DEFAULT 'draft', scheduled_at TIMESTAMPTZ, started_at TIMESTAMPTZ, ended_at TIMESTAMPTZ, max_viewers INT DEFAULT 0, current_viewers INT DEFAULT 0, total_views INT DEFAULT 0, is_password_protected BOOLEAN NOT NULL DEFAULT false, event_password TEXT, allow_chat BOOLEAN NOT NULL DEFAULT false, allow_reactions BOOLEAN NOT NULL DEFAULT true, simulcast_config JSONB DEFAULT '[]', template_id UUID, template_data JSONB DEFAULT '{}', validity_expires_at TIMESTAMPTZ, hero_image_url TEXT, header_image_url TEXT, player_image_url TEXT, photo_gallery_urls JSONB DEFAULT '[]', photographer_logo_url TEXT, photographer_contact JSONB DEFAULT '{}', crew_pin_hash TEXT, credits_consumed INT DEFAULT 1, slug TEXT UNIQUE, credit_deduction_id UUID REFERENCES credit_deductions(id), use_custom_domain BOOLEAN DEFAULT false, metadata JSONB DEFAULT '{}', created_at TIMESTAMPTZ NOT NULL DEFAULT NOW(), updated_at TIMESTAMPTZ NOT NULL DEFAULT NOW())`],
    ...
    ["ALTER events rtmp provider", `ALTER TABLE events ADD COLUMN IF NOT EXISTS rtmp_provider TEXT NOT NULL DEFAULT 'srs', ADD COLUMN IF NOT EXISTS rtmp_provider_stream_id TEXT, ADD COLUMN IF NOT EXISTS rtmp_provider_payload JSONB DEFAULT '{}'::jsonb`],
    ["IDX events_rtmp_provider", `CREATE INDEX IF NOT EXISTS idx_events_rtmp_provider ON events(rtmp_provider)`],
    ["IDX events_rtmp_provider_stream_id", `CREATE INDEX IF NOT EXISTS idx_events_rtmp_provider_stream_id ON events(rtmp_provider_stream_id)`],
    ...
    ["TABLE invoice_sequences", `CREATE TABLE IF NOT EXISTS invoice_sequences (financial_year TEXT PRIMARY KEY, next_number BIGINT NOT NULL DEFAULT 1, created_at TIMESTAMPTZ NOT NULL DEFAULT NOW(), updated_at TIMESTAMPTZ NOT NULL DEFAULT NOW(), CHECK (next_number > 0))`],
  ...
  const columnAlters: [string, string][] = [
    ...
    ["ALTER events capture_visitor_data", `ALTER TABLE events ADD COLUMN IF NOT EXISTS capture_visitor_data BOOLEAN NOT NULL DEFAULT false`],
  ...
  const clientGalleryMigrations: [string, string][] = [
    [
      "TABLE client_gallery_albums",
    ...
    [
      "TABLE client_gallery_storage_settings",
  ...
  for (const [label, stmt] of clientGalleryMigrations) {
    await tryExec(label, stmt)
  }
  ...
  const settings = [
    ...
    ["validity_extensions", { defaultDays:30, extensions:[{days:60,creditCost:1,label:"60 Days (+1 credit)",enabled:true},{days:90,creditCost:2,label:"90 Days (+2 credits)",enabled:true},{days:180,creditCost:5,label:"180 Days (+5 credits)",enabled:true},{days:365,creditCost:12,label:"365 Days (+12 credits)",enabled:true}] }],
