...
export default function StudioEventsPage() {
  ...
  useEffect(() => {
    ...
    if ((ytConnected || ytError) && saved) {
      try {
        const { openModal, tab, streamType, mode, eventId } = JSON.parse(saved)
        ...
        if (openModal) {
          if (mode === "edit" && eventId) {
            // Re-open in edit mode so we send PUT instead of POST and keep slug valid.
            // Use full event from list if available (see effect below when data loads).
            setEditingEvent({ id: eventId } as unknown as LiveEvent)
          } else {
            setEditingEvent(undefined)
          }
          setEventDialogInitialTab(tab ?? "stream")
          if (streamType) {
            setEventDialogInitialStreamType(streamType as StreamType)
          }
          setShowEventDialog(true)
        }
      } catch {
  ...
  useEffect(() => {
    if (!editingEvent?.id || !events.length) return
    const keys = Object.keys(editingEvent)
    if (keys.length > 1) return
    const full = (events as Record<string, unknown>[]).find((e) => e.id === editingEvent.id)
    if (full) setEditingEvent(full as unknown as LiveEvent)
  }, [events, editingEvent])
  ...
  const handleForceStop = async (event: Record<string, unknown>) => {
    try {
      ...
      if (!res.ok) { toast.error("Failed to stop event"); return }
  ...
  const getStatusDot = (status: string) => {
    switch (status) {
      ...
      case "on_break": return "bg-orange-400 animate-pulse"
  ...
  const getStreamIcon = (streamType: string) => {
    switch (streamType) {
      ...
      case "third_party":
      case "embedded": return <Globe className="h-4 w-4 text-blue-400" />
      default: return <Video className="h-4 w-4 text-primary" />
    }
  }
  ...
  const getThumbnailBg = (streamType: string) => {
    switch (streamType) {
      case "youtube_embed":
      case "youtube_api":
      case "youtube": return "bg-red-950/60"
      case "third_party":
      case "embedded": return "bg-blue-950/60"
  ...
  return (
    <div className="space-y-6">
      ...
      {/* Stream credentials dialog — shown only when RTMP/YouTube API is selected */}
