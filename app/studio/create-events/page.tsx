  ...
  return (
    <div className="flex flex-col h-full space-y-4">
          ...
          <div className="space-y-4 px-1">
            ...
            <div className="space-y-3">
              {statusFilters.map(filter => {
                const count = relevantEvents.filter(e => calendarStatusGroup(e.status) === filter.id).length;
                return (
      ...
      <EventDetailSheet
        ...
        dashboardHref={(event) => `/${user?.role === "studio" ? "studio" : "streamer"}/control-center/${event.id}`}
      />
    </div>
  )
}
