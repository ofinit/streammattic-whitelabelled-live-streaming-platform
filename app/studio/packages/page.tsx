...
import {
  Video, Youtube, MonitorPlay, Globe, ShoppingCart, Minus, Plus, CreditCard, TrendingDown, CheckCircle2, Wallet, Clock, Users,
} from "lucide-react"
import { masterStreamTypePricing, masterValiditySettings, mockStudios, getBestPriceForQuantity } from "@/lib/mock-data"
    ...
    <div className="space-y-6">
      <div className="flex flex-col sm:flex-row sm:items-center sm:justify-between gap-4">
        <div>
          ...
          <p className="text-muted-foreground">Purchase credits for your streamers. Buy in bulk for volume discounts.</p>
        </div>
        <Card className="border-border bg-card px-4 py-2 shrink-0">
          <div className="flex items-center gap-2">
            <Wallet className="h-4 w-4 text-primary" />
            <span className="text-sm text-muted-foreground">Wallet:</span>
            <span className="font-bold">{formatCurrency(studio.walletBalance)}</span>
          </div>
        </Card>
      </div>
      ...
      <div className="rounded-lg border border-primary/20 bg-primary/5 p-4">
        <div className="flex items-center gap-2 text-sm">
          ...
          <span>Credits purchased here are available for <span className="font-medium text-foreground">all streamers</span> in your studio to use when creating events.</span>
        </div>
      </div>
            ...
            {streamTypeInfo.filter((st) => masterStreamTypePricing[st.key].enabled).map(({ key, label, icon: Icon }) => (
              <div key={key} className="rounded-lg bg-secondary/50 p-3 flex items-center gap-3">
                ...
                <div>
                  ...
                  <p className="text-lg font-bold">{studio.credits[key]}</p>
                </div>
              </div>
            ))}
          </div>
      ...
      <div className="space-y-4">
        {streamTypeInfo.map(({ key, label, description, icon: Icon, recommended }) => {
          ...
          const walletEnough = studio.walletBalance >= totalPrice
              ...
              <CardHeader className="pb-3">
                <div className="flex items-center justify-between">
                  ...
                  <div className="text-right">
                    ...
                    <p className="text-lg font-bold">{formatCurrency(config.basePrice)}<span className="text-xs font-normal text-muted-foreground">/credit</span></p>
                  </div>
                </div>
              </CardHeader>
              <CardContent>
                      ...
                      {config.volumeDiscountTiers.map((tier) => {
                        ...
                        return (
                          <div key={tier.minQty} className={`rounded-md px-3 py-1.5 text-xs ${
                            ...
                            {tier.minQty}+ credits: {formatCurrency(tier.pricePerEvent)}/ea ({discount}% off)
                          </div>
                        )
                      })}
                    </div>
                ...
                {!walletEnough && (
                  <p className="mt-3 text-xs text-destructive">Insufficient wallet balance. You need {formatCurrency(totalPrice - studio.walletBalance)} more.</p>
                )}
              </CardContent>
            </Card>
          )
