...
export function EventStreamControlPage({ eventsListHref }: EventStreamControlPageProps) {
    ...
    async function load() {
      try {
        ...
        if (res.ok) {
          ...
          const found = data.event as LiveEvent
          setEvent(found)
          setShowRecording(!!(found as unknown as Record<string, unknown>).showRecording)
          setStream({
            id: `ns-${eventId}`,
            eventId,
            applicationName: `event-${eventId}`,
            streamName: "live",
            rtmpUrl: (found as unknown as Record<string,unknown>).rtmpUrl as string || `rtmp://stream.streamlivee.com/live`,
            streamKey: (found as unknown as Record<string,unknown>).streamKey as string || `sk_${eventId}_live`,
            hlsPlaybackUrl: (found as unknown as Record<string,unknown>).hlsUrl as string || `https://cdn.streamlivee.com/event-${eventId}/live/playlist.m3u8`,
            dashPlaybackUrl: `https://cdn.streamlivee.com/event-${eventId}/live/manifest.mpd`,
      ...
      } catch { /* leave event null */ } finally {
        setIsLoading(false)
      }
    }
    load()
  ...
  const handleTakeBreak = async () => {
    ...
    try {
      const res = await fetch("/api/studio/events", {
        ...
        body: JSON.stringify({ id: eventId, status: "on_break" }),
      })
      if (res.ok) {
        setEvent((prev) => (prev ? { ...prev, status: "on_break" as never } : prev))
        setIsOnBreak(true)
        toast({ title: "Stream on break", description: "Viewers see the BRB screen. Resume when ready." })
      } else {
        toast({ title: "Failed to pause stream", variant: "destructive" })
      }
    } catch {
      toast({ title: "Failed to pause stream", variant: "destructive" })
    }
  ...
  const handleResume = async () => {
    try {
      ...
      if (res.ok) {
        ...
        setIsOnBreak(false)
        toast({ title: "Stream resumed!", description: "You are live again." })
      } else {
        toast({ title: "Failed to resume stream", variant: "destructive" })
      }
    } catch {
      toast({ title: "Failed to resume stream", variant: "destructive" })
    }
  ...
  const handleStopStream = async () => {
    ...
    try {
      const res = await fetch("/api/studio/events", {
        method: "PUT",
        headers: { "Content-Type": "application/json" },
        body: JSON.stringify({ id: eventId, status: "ended" }),
      })
      if (res.ok) {
  ...
  const handleToggleRecording = async () => {
    ...
    try {
      const res = await fetch("/api/studio/events", {
        method: "PUT",
        headers: { "Content-Type": "application/json" },
        body: JSON.stringify({ id: eventId, showRecording: next }),
      })
      if (res.ok) {
        setShowRecording(next)
        toast({ title: next ? "Recording visible to viewers" : "Recording hidden from viewers" })
      } else {
        toast({ title: "Failed to update recording setting", variant: "destructive" })
      }
    } catch {
      toast({ title: "Failed to update recording setting", variant: "destructive" })
    }
  ...
  if (!event) {
    return (
      <div className="flex flex-col items-center justify-center min-h-screen gap-4">
        ...
        <Button variant="outline" onClick={() => router.push(eventsListHref)}>
  ...
  const isOnBreakStatus = event.status === "on_break"
  const isLive = event.status === "live" || isOnBreakStatus
  const isCompleted = event.status === "ended" || event.status === "completed"
        ...
        <div className="mx-auto max-w-7xl px-4 py-4">
          <div className="flex items-center justify-between">
            <div className="flex items-center gap-4">
              <Button variant="ghost" size="icon" onClick={() => router.push(eventsListHref)}>
            ...
            <div className="flex items-center gap-2">
              ...
              {!isLive && !isCompleted && (
                <Button onClick={() => setShowGoLiveDialog(true)} disabled={isGoingLive} className="bg-red-600 hover:bg-red-700 text-white">
              ...
              {isCompleted && (
                <Button
                  ...
                  className={showRecording ? "border-primary text-primary" : "border-border text-muted-foreground"}
                >
                  <Film className="h-4 w-4 mr-2" />
                  {showRecording ? "Recording On" : "Show Recording"}
                </Button>
              )}
            </div>
          </div>
      ...
      <AlertDialog open={showBreakDialog} onOpenChange={setShowBreakDialog}>
        <AlertDialogContent>
          <AlertDialogHeader>
            ...
            <AlertDialogDescription>
              Viewers will see a &quot;Be Right Back&quot; holding screen. Your stream key stays active — resume whenever you&apos;re ready and viewers will automatically see the live player again.
            </AlertDialogDescription>
          </AlertDialogHeader>
          <AlertDialogFooter>
            <AlertDialogCancel>Cancel</AlertDialogCancel>
            <AlertDialogAction onClick={handleTakeBreak} className="bg-orange-500 hover:bg-orange-600 text-white">
              <PauseCircle className="h-4 w-4 mr-2" />
              Take a Break
            </AlertDialogAction>
          </AlertDialogFooter>
      ...
      <AlertDialog open={showGoLiveDialog} onOpenChange={setShowGoLiveDialog}>
        <AlertDialogContent>
          <AlertDialogHeader>
            ...
            <AlertDialogDescription>
              This will make your event live and visible to all viewers immediately. Make sure your encoder (OBS/Wirecast) is connected and streaming before going live.
            </AlertDialogDescription>
          </AlertDialogHeader>
          <AlertDialogFooter>
            <AlertDialogCancel>Cancel</AlertDialogCancel>
            <AlertDialogAction onClick={handleGoLive} className="bg-red-600 hover:bg-red-700 text-white">
              <Play className="h-4 w-4 mr-2" />
