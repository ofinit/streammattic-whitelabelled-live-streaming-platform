...
export type StudioSetupWizardProps = {
  mode: "page" | "modal"
  /** Same as URL ?force=1 — required for studios that already completed setup when re-opening the wizard */
  forceSetupWizard: boolean
  /** Same as URL ?upgraded=1 — post-upgrade banner */
  upgradedBanner: boolean
  /** When mode is modal, called from Exit setup and after successful completion */
  onClose?: () => void
}
...
export function StudioSetupWizard({
  mode,
  forceSetupWizard,
  upgradedBanner,
  onClose,
  ...
  }, [mode, draftHydrated, setupLoadMeta, user?.role, forceSetupWizard, router])
          ...
          <div className="flex flex-wrap items-start justify-between gap-3">
            ...
            <Button
              ...
              onClick={() =>
                mode === "modal"
                  ? onClose?.()
                  : router.push(isStreamerSetup ? "/streamer" : "/studio")
              }
            >
              Exit setup
            </Button>
