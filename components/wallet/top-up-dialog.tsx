...
import { CreditCard, Wallet, Building, Receipt } from "lucide-react"
...
interface TopUpDialogProps {
  ...
  onConfirm: (amount: number, gateway: string) => void
  isAdmin?: boolean
...
export function TopUpDialog({
  ...
  const handleConfirm = () => {
    ...
    if (baseAmountInPaise > 0) {
      onConfirm(baseAmountInPaise, gateway)
      setAmount("")
      onOpenChange(false)
    }
  }
  ...
  return (
    <Dialog open={open} onOpenChange={onOpenChange}>
      <DialogContent className="sm:max-w-md">
        <DialogHeader>
          <DialogTitle>{isAdmin && targetUser ? `Top Up ${targetUser.name}'s Wallet` : "Top Up Wallet"}</DialogTitle>
          <DialogDescription>
          ...
          {!isAdmin && (
            <div className="space-y-2">
              ...
              <RadioGroup value={gateway} onValueChange={setGateway} className="grid grid-cols-1 gap-2">
                ...
                <label
                  className={`flex cursor-pointer items-center gap-3 rounded-lg border p-3 transition-colors ${
                    gateway === "cashfree" ? "border-primary bg-primary/5" : "border-border hover:bg-accent"
                  }`}
                >
        ...
        <DialogFooter>
          ...
          <Button onClick={handleConfirm} disabled={!amount || Number.parseFloat(amount) <= 0}>
            {isAdmin
              ? "Add Funds"
