...
import { WeddingSouthIndianArchWatchView } from "./wedding-south-indian-arch-watch-view"
import { WeddingMaroonHeritageWatchView } from "./wedding-maroon-heritage-watch-view"
...
type RtmpPlaybackSource =
  | { status: "idle" | "checking" | "waiting" | "processing"; hlsUrl?: string }
  | { status: "hls" | "mp4"; url: string }

type RtmpSourceResponse = {
  stream: string
  status: string
  hlsUrl: string
  ...
  selected: {
    type: "hls" | "mp4" | "waiting" | "processing"
    url: string | null
  }
}
...
export function WatchEventContent({ eventId }: { eventId: string }) {
  ...
  const [rtmpPlaybackSource, setRtmpPlaybackSource] = useState<RtmpPlaybackSource>({ status: "idle" })
  ...
  useEffect(() => {
    ...
    const isRtmpPlayableStatus = status === "live" || status === "ended" || status === "completed"

    if (event?.streamType !== "rtmp" || !isRtmpPlayableStatus) {
      setRtmpPlaybackSource({ status: "idle" })
      return
    }
    ...
    const loadSource = async () => {
      try {
        const res = await fetch(`/api/watch/${encodeURIComponent(eventId)}/rtmp-source`, {
          cache: "no-store",
          signal: controller.signal,
        })
        if (!res.ok) {
          setRtmpPlaybackSource({ status: isLive ? "waiting" : "processing" })
          return
        }

        const data = (await res.json()) as RtmpSourceResponse

        if (data.selected.type === "hls" && data.selected.url) {
          setRtmpPlaybackSource({ status: "hls", url: data.selected.url })
          return
        }
        if (data.selected.type === "mp4" && data.selected.url) {
          setRtmpPlaybackSource({ status: "mp4", url: data.selected.url })
          return
        }

        setRtmpPlaybackSource({
          status: data.selected.type === "waiting" ? "waiting" : "processing",
          hlsUrl: data.hlsUrl,
        })
      } catch {
    ...
    void loadSource()
    intervalId = window.setInterval(loadSource, isLive ? 3000 : 5000)

    return () => {
      controller.abort()
      if (intervalId) window.clearInterval(intervalId)
    }
  }, [event?.status, event?.streamType, eventId])
  ...
  const rtmpPlaybackMp4Url =
    rtmpPlaybackSource.status === "mp4"
      ? rtmpPlaybackSource.url
      : null
  const isRtmpLiveHlsWaiting =
    event.streamType === "rtmp" &&
    event.status === "live" &&
    (rtmpPlaybackSource.status === "checking" || rtmpPlaybackSource.status === "waiting")
  const isRtmpRecordingProcessing =
    ...
    !rtmpPlaybackMp4Url
  const hasReplay =
    (event.streamType === "rtmp" && (!!rtmpPlaybackHlsUrl || !!rtmpPlaybackMp4Url)) ||
    (showRecording && (
  ...
  if (isEnded && hasReplay) {
    if (event.streamType === "rtmp" && rtmpPlaybackMp4Url) {
      replaySrc = rtmpPlaybackMp4Url
    ...
    <div className="w-full">
      ...
      <div ref={videoContainerRef} className={shellClassName}>
          <div className="absolute inset-0 flex items-center justify-center">
            {isOnBreak ? (
            ...
            ) : isEnded && event.streamType === "rtmp" && rtmpPlaybackHlsUrl ? (
              <div className="h-full w-full [&>div]:h-full [&>div]:rounded-none">
                <StreamPlayer
                  hlsUrl={rtmpPlaybackHlsUrl}
                  isLive={false}
                  isPlayable
                  eventTitle={event.title}
                  streamType="rtmp"
                ...
                <div className="relative flex max-w-md flex-col items-center gap-3">
                  ...
                  <div className="space-y-1">
                    <p className="text-sm font-medium text-white">
                      {rtmpPlaybackSource.status === "checking" ? "Checking recording..." : "Recording is being processed"}
                    </p>
              ...
              renderMockPlayerContent()
            ) : event.streamType === "rtmp" && event.status === "live" && rtmpPlaybackHlsUrl ? (
              <div className="h-full w-full [&>div]:h-full [&>div]:rounded-none">
                <StreamPlayer
                  hlsUrl={rtmpPlaybackHlsUrl}
                  isLive
              ...
              <div className="flex flex-col items-center justify-center gap-4 text-center px-8 absolute inset-0 bg-black">
                ...
                <div className="relative flex max-w-md flex-col items-center gap-3">
                  ...
                  <div className="space-y-1">
                    <p className="text-sm font-medium text-white">Waiting for HLS manifest from streaming server...</p>
                    <p className="text-xs text-white/60">
                      OBS is publishing, but {rtmpPlaybackSource.hlsUrl || rtmpHlsUrl} is not serving a playable manifest yet.
                    </p>
                  </div>
                </div>
              </div>
  ...
  if (watchSkin === "weddingOrnateFloral") {
  ...
  if (watchSkin === "weddingSouthIndianArch") {
    const archTeaserRaw =
      typeof weddingFields.teaserYoutubeUrl === "string" ? weddingFields.teaserYoutubeUrl.trim() : ""
    const archTeaserEmbed = archTeaserRaw ? youtubeUrlToEmbed(archTeaserRaw) : null
    const archInvitationLine =
      typeof weddingFields.invitationLine === "string" && weddingFields.invitationLine.trim()
        ? weddingFields.invitationLine.trim()
        : weddingHeroDescription ||
          "We solicit your gracious virtual presence with family and friends on this auspicious occasion."
    const archEventDates = eventDates.map((d) => ({
      id: d.id,
      label: d.label,
      formatted: formatExtraDate(d),
    }))
    const archPhotographerCredit =
      photographerLogoUrl ||
      photographerContact.name ||
      photographerContact.phone ||
      photographerContact.email ||
      photographerWebsitePublicUrl ? (
        <div className="mx-auto max-w-3xl">
          ...
          {photographerContact.name ? (
            <p className="text-lg font-semibold text-[#800020]">Photography by {photographerContact.name}</p>
          ) : (
            <p className="text-lg font-semibold text-[#800020]">Photography</p>
          )}
          <div className="mt-2 flex flex-wrap justify-center gap-x-4 gap-y-1 text-sm text-[#4a0012]/75">
            {photographerContact.phone ? (
              <a href={`tel:${photographerContact.phone}`} className="hover:underline">
                {photographerContact.phone}
              </a>
    ...
    return (
      <WeddingSouthIndianArchWatchView
        globalHeaderImage={globalHeaderImage}
        heroImageUrl={heroBackdropUrl}
        coupleHero={coupleHero}
        coupleParts={coupleParts}
        ...
        allowChat={allowChat}
        showChat={showChat}
        invitationLine={archInvitationLine}
        teaserEmbed={archTeaserEmbed}
        gallerySection={
          photoGalleryUrls.length > 0 ? (
        ...
        photographerCredit={archPhotographerCredit}
      />
    )
  }

  if (watchSkin === "weddingMaroonHeritage") {
    const heritageTeaserRaw =
      typeof weddingFields.teaserYoutubeUrl === "string" ? weddingFields.teaserYoutubeUrl.trim() : ""
    const heritageTeaserEmbed = heritageTeaserRaw ? youtubeUrlToEmbed(heritageTeaserRaw) : null
    const heritageInvitationLine =
      typeof weddingFields.invitationLine === "string" && weddingFields.invitationLine.trim()
        ? weddingFields.invitationLine.trim()
        : weddingHeroDescription ||
          "We solicit your gracious virtual presence with family and friends on this auspicious occasion."
    const heritageEventDates = eventDates.map((d) => ({
      id: d.id,
      label: d.label,
      formatted: formatExtraDate(d),
    }))
    const heritagePhotographerCredit =
      photographerLogoUrl ||
      photographerContact.name ||
      photographerContact.phone ||
      photographerContact.email ||
    ...
    return (
      <WeddingMaroonHeritageWatchView
        globalHeaderImage={globalHeaderImage}
        heroImageUrl={heroBackdropUrl}
        coupleHero={coupleHero}
        coupleParts={coupleParts}
        ...
        allowChat={allowChat}
        showChat={showChat}
        invitationLine={heritageInvitationLine}
        teaserEmbed={heritageTeaserEmbed}
        gallerySection={
          photoGalleryUrls.length > 0 ? (
        ...
        photographerCredit={heritagePhotographerCredit}
      />
    )
  }
