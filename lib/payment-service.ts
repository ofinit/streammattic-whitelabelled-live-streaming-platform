...
export async function processSuccessfulPayment(params: {
  ...
  // Update order status
  await sql`UPDATE orders SET status = 'completed', updated_at = NOW() WHERE id = ${params.orderId}`
  ...
  // Get the order to determine what to do
  const orderRows = await sql`SELECT * FROM orders WHERE id = ${params.orderId}`
  if (orderRows.length === 0) return

  const order = orderRows[0] as Record<string, unknown>
  const orderType = order.order_type as string

  // Credit wallet for wallet recharges
  if (orderType === "wallet_recharge") {
    const wallets = await sql`SELECT * FROM wallets WHERE user_id = ${params.userId}`
    if (wallets.length > 0) {
      ...
      await sql`
        ...
        VALUES (${wallet.id}, ${params.userId}, 'credit', 'top_up', ${params.amount}, ${currentBalance}, ${newBalance}, 'Wallet recharge via ${params.gateway}', ${params.orderId}, 'order')
      `
    }
  }
