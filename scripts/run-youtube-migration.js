...
async function run() {
  ...
  await client.query("CREATE INDEX IF NOT EXISTS idx_youtube_channels_owner ON youtube_channels(owner_id, owner_type)");
  ...
  await client.query("CREATE INDEX IF NOT EXISTS idx_youtube_broadcasts_event ON youtube_broadcasts(event_id)");
  await client.query("CREATE INDEX IF NOT EXISTS idx_youtube_broadcasts_channel ON youtube_broadcasts(youtube_channel_id)");
  console.log("Indexes created.");

  const tables = await client.query("SELECT table_name FROM information_schema.tables WHERE table_name LIKE 'youtube_%'");
